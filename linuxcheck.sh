...
*********************************************
Linux主机安全检查:
	1.首先采集原始信息保存到/tmp/ck_${ipadd}_${date}/check_file/文件夹下
	2.将系统日志、应用日志打包并保存到/tmp/ck_${ipadd}_${date}/log/目录下
	3.在检查过程中若发现存在问题则直接输出到/tmp/ck_${ipadd}_${date}/danger_file.txt文件中
	4.有些未检查可能存在问题的需要人工分析原始文件
	5.脚本编写环境Centos7,在实际使用过程中若发现问题可以反馈给我
...
check_file="/tmp/ck_${ipadd}_${date}/check_file/"
danger_file="/tmp/ck_${ipadd}_${date}/danger_file.txt"
log_file="/tmp/ck_${ipadd}_${date}/log/"
rm -rf $check_file
rm -rf $danger_file
rm -rf log_file
mkdir /tmp/ck_${ipadd}_${date}/
...
file="/tmp/ck_${ipadd}_${date}/sysfile_md5.txt"
if [ -e "$file" ]; then 
	md5sum -c "$file" 2>&1; 
else
...
zip -r /tmp/ck_${ipadd}_${date}.zip /tmp/ck_${ipadd}_${date}/*
